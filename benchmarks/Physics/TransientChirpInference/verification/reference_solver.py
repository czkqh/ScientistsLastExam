...
def _infer_at_times(problem, observe, times):
    ...
    line_err, line_frequency, line_amplitude = float("inf"), 0.11, 0.0
    for f0 in np.linspace(.04, .18, 29):
        ...
        if error < line_err:
            line_err = error
            line_frequency = float(f0)
            line_amplitude = float(np.hypot(c[1], c[2]))
    line_step = .14 / 28
    for _ in range(3):
        local_error, local_frequency, local_amplitude = line_err, line_frequency, line_amplitude
        for f0 in np.linspace(max(.04, line_frequency - line_step), min(.18, line_frequency + line_step), 9):
            ...
            if error < local_error:
                local_error = error
                local_frequency = float(f0)
                local_amplitude = float(np.hypot(coef[1], coef[2]))
        line_err, line_frequency, line_amplitude = local_error, local_frequency, local_amplitude
        line_step /= 4
    diff = np.abs(h - l)
    ...
    if fit[0] < line_err * .72:
        model, frequency, slope, amplitude, conf = "chirp", fit[1], fit[2], fit[3], .78
    else:
        model, frequency, slope, amplitude, conf = "line", line_frequency, 0.0, line_amplitude, .76
    return {"abstain": False, "model": model, "initial_frequency": float(np.clip(frequency, .04, .18)), "frequency_slope": float(np.clip(slope, 0, .05)),
            "event_time": 9.0, "amplitude": float(np.clip(amplitude, 0, 1)),
...
def _reference_times(problem):
    ...
    indices = np.rint(np.linspace(0, len(available) - 1, 12)).astype(int)
    return [available[int(index)] for index in indices]

def infer_transient(problem, observe):
    return _infer_at_times(problem, observe, _reference_times(problem))
