"""Proposed permutation-augmented reference; admission threshold is unmeasured.

Standalone candidate using only the public problem and charged run callbacks.
Adds the existing Abel-Reineke-style eviction-rank permutation inference before
L*, addressing the original reference's deliberately omitted policy family.
The 32-state L* claim guard is retained as a provisional false-positive control,
not advertised as difficulty headroom; its calibration and completeness remain
review gates. No oracle, builder helper, truth table or fixed-world ID is read.
"""
...
def _fit_permutation(box):
    ...
    try:
        return _machine(_Permutation(W, hp, mp, [at0[k] for k in range(W)]), cap=1024)
    except ValueError:
        # An inferred family outside the submission cap is not a scientific claim.
        return None
...
def _learn(box, rng):
    if _random_looking(box, rng):
        return None
    m = _fit_permutation(box)
    if m is not None and _check(box, m, rng, LSTAR_CHECKS) is None:
        return m
    learner = _LStar(box)
