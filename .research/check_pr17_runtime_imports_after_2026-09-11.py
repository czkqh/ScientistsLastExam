...
with tempfile.TemporaryDirectory() as temporary:
 ...
 for name,source,task in [('base',base,root/'empty-task'),('qutip',qutip,Path('benchmarks/Physics/HamiltonianLearning'))]:
  ...
  row={'name':name,'candidate_sha256':hashlib.sha256(source.encode()).hexdigest(),'candidate_proxy_attempted':False}
  try:
   ...
   row['candidate_proxy_attempted']=True
   ...
   if 'preflight' not in row:
    row.update(preflight='rejected',status='preflight_rejected',preflight_error=str(error))
  rows.append(row)
...
Path('/tmp/sle-pr17-runtime-micro-after-20260911.json').write_text(json.dumps(report,indent=2)+'\n')
