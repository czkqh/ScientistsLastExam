...
| [`SpikeHistoryInference`](benchmarks/Biology/SpikeHistoryInference/)<br>神经脉冲历史核推断 | Biology | Neuroscience | clipped | physical_sim | candidate | A neuron's spike train reflects both external drive and its own recent firing. | 从重复刺激下的神经脉冲序列恢复刺激增益与不应期历史核,并拒绝爆发、潜在试次增益混合和刺激依赖不应期 | 点过程参数恢复 + 条件放电概率预测 + 模型失配归因;误发现、拒答、尝试覆盖率与留出迁移分列 |
